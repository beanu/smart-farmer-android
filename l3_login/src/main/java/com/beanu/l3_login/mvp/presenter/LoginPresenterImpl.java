...
public class LoginPresenterImpl extends LoginContract.Presenter {
    ...
    public void login(final String account, final String password, final String loginType, String token, int sex, String icon, String nickName) {
        ...
        mModel.httpLogin(account, password, loginType, token, sex, icon, nickName)
                .as(bindLifecycle())
                .subscribe(new Observer<User>() {
                    ...
                    public void onNext(@NonNull User user) {
                        ...
                        if ("0".equals(loginType)) {
                            Arad.preferences.put(Constants.P_ACCOUNT, account);
                            Arad.preferences.put(Constants.P_PWD, EncryptUtils.encryptDES2HexString(ConvertUtils.hexString2Bytes(password), Constants.DES_KEY));
                        }
                        Arad.preferences.put(Constants.P_LOGIN_TYPE, loginType);
                        Arad.preferences.put(Constants.P_User_Id, user.getId());
